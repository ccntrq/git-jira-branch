...
export const buildTicketPickerCandidates = (
  ...
  const branchByKey = new Map<string, AssociatedBranch>();
  for (const branch of branchArray) {
    // Match getAssociatedBranch: explicit-key commands also use the first
    // associated branch returned by Git.
    if (!branchByKey.has(branch.jiraKey)) {
      branchByKey.set(branch.jiraKey, branch);
    }
  }
